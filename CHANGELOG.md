...
#### Examples — completeness + consistency

The 4 example deployments under `examples/` were validation-clean but
structurally inconsistent (2 of 4 lacked a README, none had a
`terraform.tfvars.example`). v0.3.0 brings them to a uniform shape:

| Example | Pre-v0.3.0 | v0.3.0 |
|---|---|---|
| `minimal` | `main.tf` + `versions.tf` only | + `README.md` (run + verify + troubleshooting + graduation criteria) |
| `selective` | `main.tf` + `versions.tf` + `README.md` | (unchanged — already complete) |
| `production` | `main.tf` + `versions.tf` + `variables.tf` | + `README.md` + `terraform.tfvars.example` (Slack + Teams webhook templates) |
| `cloudability-complement` | `main.tf` + `versions.tf` + `README.md` | (unchanged — already complete) |

Every example now passes `terraform validate` and provides a
copy-paste-ready entry point.

#### CI/CD hardening (post-`docs-quality.yml` first-green-run)

The first run of the v0.3.0 CI surface against this repository
surfaced real issues. Each is now fixed:

- **`.lychee.toml`** *(new)* — link-checker config at the repo root.
  Documents exclusions (AWS service-authorization slugs that AWS
  rotates, `mailto:` strings, localhost URLs). Caches the link
  database with `max_cache_age = "1d"`.
- **`.markdownlint.json`** *(new)* — relaxed markdown-lint config
  tuned for the codebase's real-world docs: `MD024 siblings_only: true`
  for Keep-a-Changelog `### Added / ### Changed / ### Fixed` repeats
  per release, `MD025 false` for `docs/finops-{en,uk}.md` "Part 1..4"
  H1 anchors, `MD034 false` for bare URLs in footers, `MD036 false`
  for emphasis-as-pseudo-header, `MD040 false` for empty/mixed code
  fences. Kept enabled: `MD022`, `MD031`, `MD032`, `MD009`, `MD012` —
  these catch real authoring bugs.
- **`docs/OPERATIONAL_RUNBOOK.md`** *(re-created, 713 lines)* —
  the runbook was lost between commits and four sibling docs
  cross-linked to it. Re-authored from scratch: naming + locate-by-
  resource reference, the 5 most common operational alerts with
  diagnostic commands + resolution paths, common procedures (snooze /
  rotate webhook / replay DLQ / reserved-concurrency kill switch /
  manual Lambda invoke), quarterly health-check one-liners, Sev-1..4
  escalation matrix.
- **README badge header** — 12 badges in two rows:
  `terraform-ci` / `python-ci` / `security` / `docs-quality` /
  `scorecard` workflow statuses, plus the live OpenSSF Scorecard
  score from `scorecard.dev`, Apache-2.0 license, latest semver tag,
  Terraform ≥ 1.6, Python 3.12, `pre-commit` enabled, Conventional
  Commits. Wrapped in `<!-- markdownlint-disable MD033 MD041 -->`
  so the inline HTML doesn't fight the lint rules.
...
- **All Checkov suppression comments preserved across the module
  ...
  resolve.
- **`.github/workflows/docs-quality.yml` simplified** — the inline
  heredoc that wrote a 4-line `.markdownlint.json` into
  `$GITHUB_WORKSPACE` at job-start time is removed; the workflow
  now reads `.markdownlint.json` from the repo. Single source of
  truth.
- **`.github/dependabot.yml` — major-version GHA bumps suppressed.**
  Added `ignore: dependency-name: "*" / version-update:semver-major`
  to the `github-actions` ecosystem block. Stops the noisy
  `actions/checkout 4→6`, `hashicorp/setup-terraform 3→4`,
  `terraform-linters/setup-tflint 4→6` PR train. Same pattern
  already used for `hashicorp/aws` below. Comment explains the
  manual escalation path for CVE-driven major bumps.
...
- **`ossf/scorecard-action` pinned to `@v2.4.0`** — was `@v2`, which
  the GitHub Actions runner couldn't resolve at runtime ("unable to
  find version v2") because the upstream repo doesn't publish a
  floating `v2` tag. Two warning comments added so a future
  maintainer doesn't naively re-loosen the pin.
- **Stale AWS Service-Authorization-Reference URL in
  `docs/COMPLIANCE_NOTES.md`** — the EC2-specific page slug had been
  re-routed by AWS (404). Replaced with the stable landing-page link
  + a note that AWS rotates per-service slugs. Also excluded in
  `.lychee.toml` as belt-and-braces.
- **Lambda code lint findings (ruff)** — 4 issues cleared:
  - `cost-data-exports/lambda/health_check.py:141` — `F841` removed
    unused `last_end_str` assignment (placeholder for a Glue field
    that isn't part of the API response).
  - `idle-resource-cleanup/lambda/_shared/idle_state.py:43` —
    `F401` removed unused
    `from boto3.dynamodb.conditions import Key`.
  - `idle-resource-cleanup/lambda/_shared/idle_state.py:245` —
    `F841` removed unused `month_prefix` and cleaned up
    `sum_savings_mtd()` to be honest about being a placeholder
    pending a future Month GSI.
  - `tag-governance/lambda/untagged_cost_report.py:266` — `E702`
    split semicolon-joined `out.append("_"); out.append(ch.lower())`
    into two lines.
