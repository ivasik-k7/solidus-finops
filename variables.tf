...
variable "budgets" {
  description = <<-EOT
    Budgets keyed by stable identifier (used in resource naming). Each entry:
      scope  = "account" | "service" | "tag" | "cost_category"
      amount = number   # monthly limit in budget_currency
      target = required for scope != "account":
    ...
    All budgets fire notifications at 50/80/100% actual and 100% forecast.
    Omit a scope entirely (e.g. no entry with scope = "account") to skip it.
    Set to {} to disable the budgets module.

    Example:
      budgets = {
        account_monthly = { scope = "account", amount = 250000 }
        ec2_monthly     = { scope = "service", amount = 80000,
                            target = { service = "Amazon Elastic Compute Cloud - Compute" } }
        retail_banking  = { scope = "tag",     amount = 80000,
                            target = { tag_key = "BusinessUnit", tag_value = "retail-banking" } }
        pci_monthly     = { scope = "cost_category", amount = 50000,
  ...
  validation {
    condition = alltrue([
      ...
      contains(["account", "service", "tag", "cost_category"], v.scope)
    ])
  ...
  validation {
    ...
    error_message = "All budgets amounts must be > 0."
  }

  validation {
    condition = alltrue([
      for k, v in var.budgets :
      v.scope == "account" || v.target != null
    ])
