...
def handler(event, context):
    logger.info(
        "Discovery scan: regions=%s cpu<%s%% lookback=%sd",
        SCAN_REGIONS,
        CPU_THRESHOLD,
        LOOKBACK_DAYS,
    )
...
def _discover_ec2(
    ec2, cw, region: str, start: dt.datetime, end: dt.datetime
) -> list[dict]:
...
def _discover_rds(
    rds, cw, region: str, start: dt.datetime, end: dt.datetime
) -> list[dict]:
    candidates: list[dict] = []
...
def _publish_metrics(counts: dict):
    data = [
        {
            "MetricName": "DiscoveryCandidateCount",
            "Value": float(c),
            "Unit": "Count",
...
def _publish_digest(
    candidates: list[dict], counts: dict, errors: list, generated_at: dt.datetime
):
