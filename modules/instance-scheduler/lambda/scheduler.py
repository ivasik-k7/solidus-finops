...
ENABLE_SPOT_MANAGEMENT    = os.environ.get("ENABLE_SPOT_MANAGEMENT", "false").lower() == "true"
...
def handler(event, context):
    logger.info(
        ...
        SCAN_REGIONS, list(SCHEDULES.keys()), ENABLE_EC2,
        ENABLE_RDS_INSTANCES, ENABLE_RDS_CLUSTERS, ENABLE_ASG,
    )

    totals = {
        "managed": {"EC2": 0, "RDSInstance": 0, "RDSCluster": 0, "ASG": 0},
        "started": 0, "stopped": 0,
        "skipped_override": 0, "skipped_ceiling": 0, "skipped_exception": 0,
        "skipped_spot": 0, "scale_ambiguous": 0,
        "dry_run": 0,
        "no_change": 0, "failed": 0,
        "actions": [],
        "errors": [],
    }
    if DRY_RUN:
...
def _process_ec2(ec2, region, now_utc, totals, action_budget):
    ...
    for page in paginator.paginate(Filters=[{"Name": "tag-key", "Values": [OPT_IN_TAG_KEY]}]):
            ...
            for inst in res.get("Instances", []) or []:
                try:
                    ...
                    if is_spot and not ENABLE_SPOT_MANAGEMENT:
                        ...
                        _record_action("EC2", inst["InstanceId"], region, "skipped-spot",
                    ...
                    outcome = _evaluate_resource(
                        ...
                        start_fn=lambda iid=inst["InstanceId"]: ec2.start_instances(InstanceIds=[iid]),
                        stop_fn=lambda iid=inst["InstanceId"]: ec2.stop_instances(InstanceIds=[iid]),
                    )
                    totals["managed"]["EC2"] += 1
                    _accumulate_outcome(totals, outcome, "EC2")
                except Exception as e:
                    logger.exception("EC2 %s failed in region %s", inst.get("InstanceId"), region)
                    totals["failed"] += 1
                    totals["errors"].append({"region": region, "resource": inst.get("InstanceId"), "error": str(e)})


def _process_rds_instances(rds, region, now_utc, totals, action_budget):
    NON_ACTIONABLE = {
        "creating", "deleting", "modifying", "backing-up", "renaming",
        "maintenance", "starting", "stopping", "rebooting", "upgrading",
        "configuring-iam-database-auth", "configuring-log-exports",
        "moving-to-vpc", "resetting-master-credentials",
    }
        ...
        for db in page.get("DBInstances", []) or []:
            try:
                ...
                if status in NON_ACTIONABLE:
                    logger.info("RDS %s in transient state %s — deferring", db["DBInstanceIdentifier"], status)
                    continue
                ...
                outcome = _evaluate_resource(
                    ...
                    now_utc=now_utc,
                    action_budget=action_budget,
                    start_fn=lambda d=db["DBInstanceIdentifier"]: rds.start_db_instance(DBInstanceIdentifier=d),
                    stop_fn=lambda d=db["DBInstanceIdentifier"]: rds.stop_db_instance(DBInstanceIdentifier=d),
                )
                totals["managed"]["RDSInstance"] += 1
                _accumulate_outcome(totals, outcome, "RDSInstance")
            except Exception as e:
                logger.exception("RDS instance %s failed in region %s", db.get("DBInstanceIdentifier"), region)
                totals["failed"] += 1
                totals["errors"].append({"region": region, "resource": db.get("DBInstanceIdentifier"), "error": str(e)})


def _process_rds_clusters(rds, region, now_utc, totals, action_budget):
    NON_ACTIONABLE = {
        "creating", "deleting", "modifying", "backing-up", "starting", "stopping",
        "rebooting", "upgrading", "maintenance", "renaming", "promoting",
        "failing-over", "configuring-iam-database-auth",
    }
        ...
        for cl in page.get("DBClusters", []) or []:
            try:
                ...
                if cl.get("EngineMode", "") == "serverless":
                    logger.info("RDS cluster %s is serverless — stop/start unsupported, skipping", cl["DBClusterIdentifier"])
                    continue
                status = cl["Status"]
                if status in NON_ACTIONABLE:
                    logger.info("RDS cluster %s in transient state %s — deferring", cl["DBClusterIdentifier"], status)
                    continue
                outcome = _evaluate_resource(
                    ...
                    now_utc=now_utc,
                    action_budget=action_budget,
                    start_fn=lambda c=cl["DBClusterIdentifier"]: rds.start_db_cluster(DBClusterIdentifier=c),
                    stop_fn=lambda c=cl["DBClusterIdentifier"]: rds.stop_db_cluster(DBClusterIdentifier=c),
                )
                totals["managed"]["RDSCluster"] += 1
                _accumulate_outcome(totals, outcome, "RDSCluster")
            except Exception as e:
                logger.exception("RDS cluster %s failed in region %s", cl.get("DBClusterIdentifier"), region)
                totals["failed"] += 1
                totals["errors"].append({"region": region, "resource": cl.get("DBClusterIdentifier"), "error": str(e)})
    ...
    for page in paginator.paginate():
        for group in page.get("AutoScalingGroups", []) or []:
            try:
                ...
                logger.exception("ASG %s failed in region %s", group.get("AutoScalingGroupName"), region)
                totals["failed"] += 1
                totals["errors"].append({"region": region, "resource": group.get("AutoScalingGroupName"), "error": str(e)})
...
def _asg_scale_to_zero(asg, group: dict):
    ...
    asg.create_or_update_tags(Tags=[
        {"ResourceId": name, "ResourceType": "auto-scaling-group", "Key": "FinOpsSavedMin",
         "Value": str(min_size), "PropagateAtLaunch": False},
        {"ResourceId": name, "ResourceType": "auto-scaling-group", "Key": "FinOpsSavedDesired",
         "Value": str(desired), "PropagateAtLaunch": False},
    ])
...
def _asg_scale_up(asg, group: dict, totals: dict | None = None):
    ...
    if not stash_present and totals is not None:
        ...
        logger.warning("ASG %s missing scale stash tags — scaling to 1/1 (safe fallback)", name)
    try:
    ...
    asg.update_auto_scaling_group(AutoScalingGroupName=name, MinSize=saved_min, DesiredCapacity=saved_desired)
    if stash_present:
...
def _evaluate_resource(
    *,
    ...
    if EXCEPTION_TAG_KEY in tags:
        _upsert_state(resource_type, resource_id, region, schedule_name, owner, actual_running, "excepted", tags)
    ...
    if override_until:
        try:
            ...
            if now_utc < until:
                _upsert_state(resource_type, resource_id, region, schedule_name, owner, actual_running, "override-active", tags)
                _record_action(resource_type, resource_id, region, "skipped-override", schedule_name, f"override until {override_until}")
                return {"outcome": "skipped_override"}
        except ValueError:
            logger.warning("Invalid %s on %s: %r", OVERRIDE_UNTIL_TAG_KEY, resource_id, override_until)

    schedule = SCHEDULES.get(schedule_name)
    if not schedule:
        logger.warning("Resource %s has Schedule=%s but no such schedule defined", resource_id, schedule_name)
        _upsert_state(resource_type, resource_id, region, schedule_name, owner, actual_running, "unknown-schedule", tags)
        return {"outcome": "no_change"}

    desired = _desired_running(schedule, now_utc)
    if desired == actual_running:
        _upsert_state(resource_type, resource_id, region, schedule_name, owner, actual_running, "in-sync", tags)
        return {"outcome": "no_change"}
    ...
    if action_budget["used"] >= MAX_ACTIONS_PER_TICK:
        logger.warning("Action ceiling (%d) reached — deferring %s to next tick",
                       MAX_ACTIONS_PER_TICK, resource_id)
        _upsert_state(resource_type, resource_id, region, schedule_name, owner, actual_running, "ceiling-skipped", tags)
        _record_action(resource_type, resource_id, region, "skipped-ceiling", schedule_name,
    ...
    if DRY_RUN:
        logger.info("DRY_RUN: would %s %s (%s) per schedule=%s", action_type, resource_id, resource_type, schedule_name)
        _upsert_state(resource_type, resource_id, region, schedule_name, owner, actual_running, "dry-run", tags)
        _record_action(resource_type, resource_id, region, f"dry-run-{action_type}",
                       schedule_name, f"DRY_RUN — would {action_type}")
    ...
    try:
        ...
        _record_action(resource_type, resource_id, region, "failed", schedule_name, f"{action_type}: {e}")
        return {"outcome": "failed"}

    action_budget["used"] += 1
    _upsert_state(resource_type, resource_id, region, schedule_name, owner, desired, "acted", tags)
...
def _accumulate_outcome(totals: dict, outcome: dict, resource_type: str):
    ...
    if o == "started":
        totals["started"] += 1
        totals["actions"].append({"action": "started", "resource_type": resource_type, **outcome})
    elif o == "stopped":
        totals["stopped"] += 1
        totals["actions"].append({"action": "stopped", "resource_type": resource_type, **outcome})
        ...
        totals["dry_run"] += 1
        totals["actions"].append({"action": "dry-run", "resource_type": resource_type, **outcome})
    else:
        totals["no_change"] += 1
...
def _epoch(days_from_now: int) -> int:
    return int((dt.datetime.now(dt.timezone.utc) + dt.timedelta(days=days_from_now)).timestamp())


def _upsert_state(resource_type, resource_id, region, schedule_name, owner, is_running, last_outcome, tags):
    ddb.put_item(Item={
...
def _record_action(resource_type, resource_id, region, action_type, schedule_name, notes):
...
def _publish_aggregate_metrics(totals: dict):
    data: list[dict[str, Any]] = [
        {"MetricName": "ActionStarted",         "Value": float(totals["started"]),           "Unit": "Count"},
        {"MetricName": "ActionStopped",         "Value": float(totals["stopped"]),           "Unit": "Count"},
        {"MetricName": "ActionSkippedOverride", "Value": float(totals["skipped_override"]),  "Unit": "Count"},
        {"MetricName": "ActionSkippedCeiling",  "Value": float(totals["skipped_ceiling"]),   "Unit": "Count"},
        {"MetricName": "ActionSkippedSpot",     "Value": float(totals["skipped_spot"]),      "Unit": "Count"},
        {"MetricName": "ActionScaleAmbiguous",  "Value": float(totals["scale_ambiguous"]),   "Unit": "Count"},
        {"MetricName": "ActionDryRun",          "Value": float(totals["dry_run"]),           "Unit": "Count"},
        {"MetricName": "ActionFailed",          "Value": float(totals["failed"]),            "Unit": "Count"},
    ]
    for rtype, count in totals["managed"].items():
        data.append({
            "MetricName": "ManagedResourceCount", "Value": float(count), "Unit": "Count",
...
def _publish_digest(totals: dict, now_utc: dt.datetime):
    ...
    if totals["started"] == 0 and totals["stopped"] == 0 and totals["failed"] == 0 and not totals["errors"]:
        return  # quiet tick — don't spam the events bus

    severity = "high" if totals["failed"] > 0 or totals["errors"] else (
        "medium" if (totals["started"] + totals["stopped"]) > 50 else "info"
    )
    summary = {
